...
package ch.usi.inf.l3.piuma.neve
      ...
      val expandee = inputs match {
        ...
        case (clazz: ClassDef) :: Nil => 
         // Cannot extend anything except AnyRef, and you should
          // remove it
          // val newParents = 
          //   q"""ch.usi.inf.l3.piuma.plugin.InfoTransformerPluginComponent(
          //       plgn)""" :: checkParents(c)(clazz.impl.parents)

          val newParents = 
            Apply(Select(q"ch.usi.inf.l3.piuma.plugin", 
          ...
          val plgnType = plgnName.equals("") match {
            case true => Select(q"ch.usi.inf.l3.piuma.plugin", 
                                  TypeName("PiumaPlugin"))
    ...
    def checkerImpl(c: Context)(annottees: c.Expr[Any]*): c.Expr[Any] = {
      ...
      val expandee = inputs match {
        case (clazz: ClassDef) :: Nil => 
          ...
          // remove it
          // val newParents = q"""
          // ch.usi.inf.l3.piuma.plugin.CheckerPluginComponent(
              // plgn)""" :: checkParents(c)(clazz.impl.parents)
    ...
    def phaseImpl(c: Context)(annottees: c.Expr[Any]*): c.Expr[Any] = {
      ...
      val expandee = inputs match {
        case (clazz: ClassDef) :: Nil => 
          ...
          // remove it
          // val newParents =
          //  q"""ch.usi.inf.l3.piuma.plugin.TransformerPluginComponent(
          //     plgn)""" :: checkParents(c)(clazz.impl.parents)
    ...
    def pluginImpl(c: Context)(annottees: c.Expr[Any]*): c.Expr[Any] = {
      ...
      val expandee = inputs match {
        case (clazz: ClassDef) :: Nil => 
          ...
          // val newParents q"ch.usi.inf.l3.piuma.plugin.PiumaPlugin(global)" ::
          //   checkParents(c)(clazz.impl.parents)

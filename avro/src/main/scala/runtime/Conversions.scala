...
trait HasTraversableConversions {
  ...
  implicit def toMap[M0 <: Traversable[(K0, V0)], K0, V0, K1, V1]
  ...
  implicit def fromScalaTraversable[FromColl <: Traversable[FromElem], FromElem, ToElem]
    ...
    def apply(s: Schema, a: FromColl) = {
      ...
      if (a eq null) null
        ...
        new GenericArray[ToElem] {
          import scala.collection.JavaConversions._
          def add(elem: ToElem) = sys.error("ADD NOT SUPPORTED - ARRAY IS IMMUTABLE")
          def clear = sys.error("CLEAR NOT SUPPORTED - ARRAY IS IMMUTABLE")
          def peek = sys.error("PEEK NOT SUPPORTED")
          def size = res.size
          def iterator = 
          ...
          // Members declared in org.apache.avro.generic.GenericArray
          def reverse(): Unit = ???
          // Members declared in java.util.List
          def add(x$1: Int,x$2: ToElem): Unit = ???
          def addAll(x$1: Int,x$2: java.util.Collection[_ <: ToElem]): Boolean = ???
          def addAll(x$1: java.util.Collection[_ <: ToElem]): Boolean = ???
          def contains(x$1: Any): Boolean = ???
          def containsAll(x$1: java.util.Collection[_]): Boolean = ???
          def get(x$1: Int): ToElem = ???
          def indexOf(x$1: Any): Int = ???
          def isEmpty(): Boolean = ???
          def lastIndexOf(x$1: Any): Int = ???
          def listIterator(x$1: Int): java.util.ListIterator[ToElem] = ???
          def listIterator(): java.util.ListIterator[ToElem] = ???
          def remove(x$1: Int): ToElem = ???
          ...
          def set(x$1: Int,x$2: ToElem): ToElem = ???
          def subList(x$1: Int,x$2: Int): java.util.List[ToElem] = ???

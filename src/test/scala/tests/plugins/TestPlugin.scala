...
class TestPluginComponent(plgn: TestPlugin) extends TransformerPluginComponent(plgn) {
  ...
  override val runsRightAfter = Some(utilities.PHASE_SUPERACCESSORS)
  val runsAfter = List[String](utilities.PHASE_FLATTEN, utilities.PHASE_PATMAT, utilities.PHASE_TYPER)
  override val runsBefore = List[String](utilities.PHASE_ICODE)
  val phaseName = "test"
...
class TestPluginComponent2(plgn: TestPlugin) extends TransformerPluginComponent(plgn) {
  ...
  override val runsRightAfter = Some(utilities.PHASE_INLINER)
  override val runsAfter = List[String](utilities.PHASE_INLINER)
  val phaseName = "test2"
